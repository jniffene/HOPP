...
# def flt_pv_cost(P_cap_kW):
P_cap_kW = 15000 #input

P_cap = P_cap_kW/1000 # capacity in MW
print("System Size = ", P_cap, "MW")
I_int = P_cap * 1.28524387305415 * 1000000 # initial investment
print("Initial Investment = $", I_int)
I_1st = 0.04 * 0.2 * 10 * 10 ** 6 # 1st follow on investment
print("1st Follow On Investment = $", I_1st)
...
print("Real Equity Rate =", r_ereal*100, "%")
r_tax = 0.21 + 0.06 * (1 - 0.21) # tax rate
print("Tax Rate =", r_tax*100, "%")
...
# lcoss_num = (I_int * (1-Lev) - I_int * itc + I_1st/(1 + r_ereal)**n_1st + I_2nd/(1 + r_ereal)**n_2nd - r_tax * npf.npv(r_enom, D_ii) - r_tax * npf.npv(r_enom, D_1st) - r_tax * npf.npv(r_enom, D_2nd) + (1 - r_tax)*(npf.npv(r_enom, OMy) + npf.npv(r_enom, C_ip) + npf.npv(r_ereal, C_c) + (v_res/(1+v_res))**30) + npf.npv(r_enom, C_pp))
# print(lcoss_num)
# lcoss_den = ((1-r_tax)*(npf.npv(r_ereal, E_pv) + npf.npv(r_ereal, E_bat) + (1-Q_grid)*npf.npv(r_ereal, Elect_grid)))
# print(lcoss_den)

# npv1 = npf.npv(r_enom, D_ii)
# print(npv1)