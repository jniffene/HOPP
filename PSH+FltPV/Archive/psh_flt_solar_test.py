...
A_ca = 1.53 # m2
n_array = 0.2055 # efficiency of array (determined previously)

# Calculate capacity of floating solar system (kW)
P_cap = n_array * max(L_irr) * A_real/1000
...
# print(P_pv)
# plt.figure(1)
# plt.plot(t,P_pv,color="red")
# plt.plot(t,P_l,color="blue")
# plt.xlabel("Hour", fontsize = 14)
# plt.ylabel("Power (kW)", fontsize = 14)
# plt.title("P_pv & P_l Check")
# plt.grid(True)
# plt.show()
...
def est_hV(V_t_1, P_p_kW, P_g_kW, h_o, P_cap_kW):
    # Conver kW to W
    P_p = P_p_kW * 1000
    P_g = P_g_kW * 1000
    P_kW = P_cap_kW * 1000
...
h_t[0] = 0.0012098 * V_t[0] ** 0.99887 + h_o
...
for i in range(len(t)):
    ...
    if P_pv[i] < P_l[i]:
        if V_t[i] > 0:
            ...
            P_pv2psh[i] = 0
            P_pv2grid[i] = 0
            V_t[i+1] = est_hV(V_t[i], 0, P_l[i] - P_pv[i], h_o, P_kW)[1]
            if V_t[i+1] > 0:
    ...
    hV = est_hV(V_t[i], P_pv2psh[i] + P_grid2psh[i], P_psh2L[i], h_o, P_kW)
    h_t[i+1] = hV[0]
    V_t[i+1] = hV[1]
    h_errors[i+1] = hV[2]
