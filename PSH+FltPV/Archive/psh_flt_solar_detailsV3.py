...
from psh_cost_fxnV4 import psh_costV4
from flt_pv_cost_fxnV2 import fltPVcostV2
from statistics import mean


def psh_flt_pvDetailsV3(A_total, z_max, h_o, P_kW):
    ...
    f_pvpsh = A_real/A_total
    print("Percent of usable PV area", f_pvpsh*100, "%")
    ...
    print("Capacity of Floating PV =", P_c, "kW")
    ...
    for i in range(len(t)):
        ...
        V_t[i+1] = hV[1]
        h_errors[i+1] = hV[2]
        E_psh[i] = (P_pv2psh[i] + P_psh2L[i])*1 # times 1 since the data is hourly
    ...
    print("Target Utilization Factor =", psh_req_frac*100, "%")

    # print("Percent of hours where grid must provide power = ", grid_req_frac * 100, "%")
    # Calculate factors for PSH power production
    ...
    print("Min PSH Power Generated =", P_G_min * 1000, "kW")
    P_G_avg = mean(Pos_psh2L)/1000 # going from kW to MW
    print("Avg PSH Power Generated =", P_G_avg * 1000, "kW")
    P_G_max = max(Pos_psh2L)/1000 # going from kW to MW
    print("Max PSH Power Generated =", P_G_max * 1000, "kW")

    psh_costs = psh_costV4(h_o, A_total, z_max, P_G_min, P_G_avg, P_kW/1000) # since the capacity of the motor/generator is the max power that could be generated by the plant
    print("Total PSH Cost ($/kW) =", psh_costs[1])
    print("Total PSH Cost ($/kWh) =", psh_costs[2])
    print("PSH Capital Cost = $", psh_costs[0])
    pv_costs = fltPVcostV2(P_c)
    pv_lcoe = pv_costs[0]
    pv_capitalCost = pv_costs[1]
    print("Floating PV Cost ($/kWh) =", pv_lcoe)
    print("Floating PV Capital Cost = $", pv_capitalCost)
    ...
    max_excess_ratio = max_pv2grid/max_load
    print("Max Excess Ratio =", max_excess_ratio)

    # Utilization factor
    Uf = sum(E_psh)/(P_kW * max(t))
    print("Utilization Factor =", Uf*100, "%")
    ...
    plt.legend(["PV-G","PSH-L","PV-L", "G-PSH","PV-PSH","G-L"])
    plt.title("Power Transfer Schedule")
    plt.grid(True)
    plt.show()

    plt.figure(2)
    plt.plot(t,P_pv,color="cyan")
    plt.plot(t,P_l,color="green")
    # making a new version of t that includes 0
    t_V = np.empty(len(t)+1)
    t_V[0] = 0
    for i in range(len(t)):
        t_V[i+1] = t[i]
    plt.plot(t_V, V_t/V_max * 100, color="orange")
    plt.xlabel("Hour", fontsize = 14)
    plt.ylabel("Power (kW) & Volume (%)", fontsize = 14)
    plt.legend(["PV Generation","Load", "% Volume"])
    plt.title("Variations of reservoir volume, PV generation, and load")
    plt.grid(True)
    plt.show()
    return 

# Results = psh_flt_pvDetailsV2(1849,2.6,21.6,73.2)
