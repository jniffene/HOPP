...
from psh_flt_solar_fxnV10 import psh_flt_pvFXNv10
...
from psh_flt_solar_detailsV3 import psh_flt_pvDetailsV3
...
class pvFpshOpt(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        [f1, f2, f3] = psh_flt_pvFXNv10(x[0], x[1], x[2], x[3])
        out["F"] = [f1,f2,f3] 
...
print("PSH + PV Capital Cost = $", F[i,2])
...
ax.set_zlabel('PSH + PV Capital Cost ($)')
plt.show()

print("Additional Details on Design:")
psh_flt_pvDetailsV3(X[i,0],X[i,1],X[i,2], X[i,3])
