...
A_total = 5329.47 #m2 area of reservoir
z_max = 7.7 # m max depth of reservoir

# Possibe other variables to optimize
h_o = 16.48 # m static head (distance between reservoirs)

# Adjust the motor capacity to be ~25% of the total solar capacity (55/212.5 in Shyam)
P_kW = 2.38 # kW (Max load of generator/motor)
...
A_real = A_total - np.sqrt(2 * math.pi * A_total * A_ca)
print("PV area =", A_real, "m2")
# A_real = A_total * n_use # area useable by PV
a_p = 0.47/100 # %/C
T_ref = 25 #C
...
for i in range(len(t)):
    ...
    V_t[i+1] = hV[1]
    h_errors[i+1] = hV[2]
    E_psh[i] = (P_pv2psh[i] + P_grid2psh[i] + P_psh2L[i])*1 # times 1 since the data is hourly
...
plt.legend(["PV-G","G-PSH","G-L","PV-PSH","PV-L","PSH-L"])
