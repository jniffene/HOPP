...
def psh_flt_pvFXNv10(A_total, z_max, h_o, P_kW):
    ...
    P_mw = P_kW/1000 # going from kW to MW (This is more representative of the maximum capacity of the system)
    
    
    #print("Percent of hours where grid must provide power = ", grid_req_frac * 100, "%")
    psh_costs = psh_costV4(h_o, A_total, z_max, P_G_min, P_G_avg, P_mw)
    ...
    if total_lcoe == 10 ** 9 or total_cap_cost == 10 ** 9:
        grid_req_frac = 1
        target_uf = 1

    return grid_req_frac, total_lcoe, total_cap_cost
