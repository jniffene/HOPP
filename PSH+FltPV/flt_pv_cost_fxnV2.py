...
def fltPVcostV2(P_cap_kW):
    ...
    # Capital cost - based on results from Vignesh's excel document
    if P_cap < 0.2:
        capital_rate = 3.3584 # $/Wdc from Vignesh (these values are below the measured range)
    elif P_cap > 10:
        capital_rate = 1.7605 # $/Wdc from Vignesh (these values are above the measured range)
    else:
        capital_rate = 2.444 * P_cap ** -0.151 # $/Wdc based on calculated values from Vignesh (R2 of 0.9394)
    
    # capital cost for floating PV
    capital_cost = capital_rate * P_cap_kW * 1000 

    return lcoss_kwh, capital_cost
