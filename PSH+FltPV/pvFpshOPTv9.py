...
from psh_flt_solar_fxnV9 import psh_flt_pvFXNv9
...
from psh_flt_solar_detailsV2 import psh_flt_pvDetailsV2
...
class pvFpshOpt(ElementwiseProblem):
    def __init__(self):
        super().__init__(n_var=3,
                        ...
                        n_ieq_constr = 0,
                        xl=np.array([1,1,1]),
                        xu=np.array([A_max, z_max, h_max]))
    def _evaluate(self, x, out, *args, **kwargs):
        [f1, f2, f3, f4] = psh_flt_pvFXNv9(x[0], x[1], x[2])
        out["F"] = [f1,f2,f3,f4] 
...
print(f"Range f3 (Capital Cost $): [{fl[2]}, {fu[2]}]")
...
print("Hybrid LCOE ($/kWh) =", F[i,1])
print("Hybrid Capital Cost = $", F[i,2])
...
ax.set_ylabel('Hybrid LCOE ($/kwh)')
ax.set_zlabel('Hybrid Capital Cost ($)')
plt.show()

print("Additional Details on Design:")
psh_flt_pvDetailsV2(X[i,0],X[i,1],X[i,2])
